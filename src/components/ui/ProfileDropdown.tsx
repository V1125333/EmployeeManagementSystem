    ...
    <button
      ...
      className={cn(
        ...
        variant === 'danger'
          ...
          : 'text-[var(--color-brand-navy)] hover:bg-hover-bg focus-visible:bg-hover-bg'
      )}
    >
      <span
        className={cn(
    ...
    <div ref={dropdownRef} className="relative">
      <button
        ...
        className={cn(
          ...
          isSidebar && 'w-full items-start border border-transparent px-3 py-3 text-left hover:border-[var(--color-border)] hover:bg-hover-bg/70 hover:shadow-sm',
          isCollapsed && 'h-10 w-10 justify-center border border-transparent p-0 hover:border-[var(--color-border)] hover:bg-hover-bg',
          isOpen && cn('bg-hover-bg ring-1 ring-olive/20 shadow-sm', isSidebar && 'rounded-t-none border-[var(--color-border)]')
        )}
      >
        ...
        {isSidebar && (
          <div className="min-w-0 flex-1 text-left">
            <div className="break-words text-[13.5px] font-bold leading-snug text-[var(--color-brand-navy)]">
              {user.name}
            </div>
        ...
        {!isCollapsed && !isSidebar && (
          <div className="min-w-0 flex-1 text-left">
            <div className="truncate text-[12.5px] font-semibold text-[var(--color-brand-navy)]">{user.name}</div>
      ...
      {isOpen && (
        <div
          ...
          className={cn(
            ...
            panelPosition,
            panelWidth,
            'z-50 overflow-hidden border border-[var(--color-border)] bg-warm-card',
        ...
        >
          <div className="border-b border-[var(--color-border)] bg-gradient-to-b from-white to-warm-card px-4 py-3.5">
            <div className="flex items-center gap-3">
              <Avatar initials={user.initials} size="lg" variant="filled" src={user.profileImageUrl} />
              <div className="min-w-0">
                <div className="break-words text-[15px] font-semibold leading-snug text-[var(--color-brand-navy)]">{user.name}</div>
