...
const categoryStyles: Record<string, { dot: string; pill: string; label: string }> = {
  general: { dot: 'var(--color-brand-orange)', pill: 'bg-olive/10 text-olive', label: 'General' },
  hr: { dot: 'var(--color-brand-navy)', pill: 'bg-[var(--color-brand-navy)]/12 text-[var(--color-text-muted)]', label: 'HR' },
  policy: { dot: 'var(--color-brand-orange)', pill: 'bg-[var(--color-brand-orange)]/14 text-[var(--color-brand-orange)]', label: 'Policy' },
  urgent: { dot: 'var(--color-brand-orange)', pill: 'bg-[var(--color-brand-orange)]/14 text-[var(--color-brand-orange)]', label: 'Urgent' },
};
...
function AnnouncementFormSection({
  ...
  return (
    <section className="rounded-2xl border border-[var(--color-border)] bg-warm-card p-5 shadow-card">
      <div className="mb-5 flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-olive/10 text-olive">{icon}</div>
        <div>
          <h3 className="text-[15px] font-bold text-[var(--color-brand-navy)]">{title}</h3>
...
function DeliveryOptionCard({
  ...
  return (
    <label className={cn('flex min-h-[78px] cursor-pointer items-start gap-3 rounded-xl border px-3.5 py-3 transition-all', checked ? 'border-olive/40 bg-olive/5' : 'border-[var(--color-border)] bg-warm-bg hover:border-olive/25', disabled && 'cursor-not-allowed opacity-55')}>
      <input type="checkbox" checked={checked} disabled={disabled} onChange={(e) => onChange(e.target.checked)} className="mt-1 accent-olive" />
      <span>
        <span className="block text-[13px] font-bold text-[var(--color-brand-navy)]">{label}</span>
...
function CreateAnnouncementDrawer({
  ...
  const renderAudienceTargets = () => {
    ...
    return (
      <div className="space-y-2">
        ...
        />
        <div className="max-h-44 overflow-y-auto rounded-xl border border-[var(--color-border)] bg-warm-bg">
          {filteredEmployees.length > 0 ? filteredEmployees.map((employee) => {
            const fullName = `${employee.first_name} ${employee.last_name}`;
            return (
              <label key={employee.id} className="flex items-center gap-3 px-3 py-2.5 border-b border-[var(--color-border)] last:border-b-0 text-[13px] cursor-pointer hover:bg-hover-bg">
                <input
                  type="checkbox"
                ...
                <span className="min-w-0">
                  <span className="block font-semibold text-[var(--color-brand-navy)] truncate">{fullName}</span>
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex justify-end">
      <div className="absolute inset-0 bg-[var(--color-brand-navy)]/35 backdrop-blur-[3px]" onClick={onClose} />
      <style>{`.input-control{width:100%;padding:11px 14px;border-radius:12px;border:1px solid var(--color-border);background:var(--color-brand-canvas);color:var(--color-brand-navy);font-size:14px;font-weight:500;outline:none}.input-control:focus{border-color:rgb(102 120 95 / .45);box-shadow:0 0 0 3px rgb(102 120 95 / .1)}.announcement-workspace{width:min(100vw,800px)}@media (min-width:1024px){.announcement-workspace{width:52vw;min-width:720px}}`}</style>
      <aside className="announcement-workspace fixed right-0 top-0 flex h-screen flex-col border-l border-[var(--color-border)] bg-warm-bg shadow-[-18px_0_45px_rgba(47,52,55,0.16)] animate-[slideInRight_0.3s_cubic-bezier(0.16,1,0.3,1)]">
        <header className="sticky top-0 z-10 border-b border-[var(--color-border)] bg-warm-card/95 px-7 py-6 backdrop-blur">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-[var(--color-brand-navy)]">{editing ? 'Edit Announcement' : 'Create Announcement'}</h2>
          ...
          <div className="mx-auto max-w-[780px] space-y-5">
            <AnnouncementFormSection icon={<FileText size={17} />} title="Announcement Content" subtitle="Craft the message employees will see on their dashboard.">
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Announcement Title</label>
                ...
                <div>
                  <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Description / Rich Text Area</label>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Announcement Type</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Priority</label>
            ...
            <AnnouncementFormSection icon={<Users size={17} />} title="Audience Selection" subtitle="Choose exactly who should receive this communication.">
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                {AUDIENCES.map((audience) => (
                  <button key={audience} onClick={() => update('audience_type', audience)} className={cn('rounded-xl border px-3 py-3 text-left text-[13px] font-bold transition-all', form.audience_type === audience ? 'border-olive/40 bg-olive/10 text-olive' : 'border-[var(--color-border)] bg-warm-bg text-[var(--color-brand-navy)] hover:border-olive/30')}>
            ...
            <AnnouncementFormSection icon={<CalendarClock size={17} />} title="Scheduling" subtitle="Publish immediately or schedule the announcement for later.">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label className={cn('rounded-xl border px-4 py-3 transition-all', form.schedule_mode === 'now' ? 'border-olive/40 bg-olive/10' : 'border-[var(--color-border)] bg-warm-bg')}>
                  <input type="radio" checked={form.schedule_mode === 'now'} onChange={() => update('schedule_mode', 'now')} className="mr-2 accent-olive" />
                  <span className="text-[13px] font-bold text-[var(--color-brand-navy)]">Publish Now</span>
                </label>
                <label className={cn('rounded-xl border px-4 py-3 transition-all', form.schedule_mode === 'later' ? 'border-olive/40 bg-olive/10' : 'border-[var(--color-border)] bg-warm-bg')}>
                  <input type="radio" checked={form.schedule_mode === 'later'} onChange={() => update('schedule_mode', 'later')} className="mr-2 accent-olive" />
                  <span className="text-[13px] font-bold text-[var(--color-brand-navy)]">Schedule Later</span>
                </label>
              </div>
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Publish Date/Time</label>
                ...
                <div>
                  <label className="mb-1.5 block text-[13px] font-semibold text-[var(--color-brand-navy)]">Expiry Date</label>
            ...
            <AnnouncementFormSection icon={<Paperclip size={17} />} title="Attachments" subtitle="Attach supporting files for employees to reference.">
              <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[var(--color-border)] bg-warm-bg px-4 py-8 text-center transition-all hover:border-olive/40 hover:bg-olive/5">
                <Upload size={22} className="text-olive" />
                <span className="mt-3 text-[13px] font-bold text-[var(--color-brand-navy)]">Upload PDF, DOCX, or images</span>
              ...
              {form.attachments.length > 0 && (
                <div className="mt-3 space-y-2">
                  {form.attachments.map((file) => (
                    <div key={file} className="flex items-center gap-2 rounded-xl border border-[var(--color-border)] bg-warm-bg px-3 py-2 text-[13px] font-medium text-[var(--color-brand-navy)]">
            ...
            <AnnouncementFormSection icon={<Eye size={17} />} title="Live Preview" subtitle="Review how the announcement will appear to employees.">
              <div className="rounded-2xl border border-[var(--color-border)] bg-warm-bg p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <h4 className="text-[16px] font-bold text-[var(--color-brand-navy)]">{form.title || 'Announcement title preview'}</h4>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--color-border)] pt-3 text-[12px] text-gray-400">
        ...
        <footer className="sticky bottom-0 z-10 border-t border-[var(--color-border)] bg-warm-card/95 px-7 py-4 backdrop-blur">
      ...
      {criticalPinned && (
        <div className="mb-4 rounded-xl border border-status-error/20 bg-status-error/5 px-4 py-3 flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[13px] text-[var(--color-brand-navy)] truncate">{criticalPinned.message}</div>
          </div>
        </div>
      )}
      ...
      <Card>
        ...
        <div className="p-4">
          {loading ? (
            ...
            <div className="flex items-center justify-between gap-4 rounded-xl border border-dashed border-[var(--color-border)] bg-warm-bg px-4 py-4">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-olive/10 text-olive">
                  <Megaphone size={18} />
                </div>
                <div className="min-w-0">
                  <div className="text-[14px] font-bold text-[var(--color-brand-navy)]">No active announcements</div>
            ...
            <div className="space-y-2">
              {announcements.slice(0, 5).map((announcement) => (
                <div key={announcement.id} className={cn(
                  ...
                  announcement.is_pinned ? 'border-olive/30' : 'border-[var(--color-border)]'
                )}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        {announcement.is_pinned && <Pin size={13} className="text-olive shrink-0" />}
                        <h3 className="text-[14px] font-bold text-[var(--color-brand-navy)] truncate">{announcement.title}</h3>
                      </div>
