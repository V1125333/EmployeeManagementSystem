...
function formatAuditValue(value: unknown): string {
  ...
  if (typeof value === 'object') {
    ...
    return entries.length
      ? entries.map(([key, item]) => `${labelize(key)}: ${formatAuditValue(item)}`).join(' · ')
      : 'None';
  }
  return labelize(String(value));
}

function ChangedFields({ fields }: { fields: NonNullable<AuditLogRow['changed_fields']> }) {
  return (
    <div className="divide-y divide-[var(--color-border)] overflow-hidden rounded-xl border border-[var(--color-border)]">
      {Object.entries(fields).map(([field, change]) => (
        <div key={field} className="p-4">
          <div className="mb-2 text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400">{labelize(field)}</div>
          <div className="grid items-center gap-2 sm:grid-cols-[1fr_auto_1fr]">
            <div className="rounded-lg bg-warm-bg px-3 py-2">
              <div className="text-[10px] font-bold uppercase tracking-wide text-gray-400">Previous</div>
              <div className="mt-0.5 break-words text-sm font-semibold text-gray-600">{formatAuditValue(change.old)}</div>
            </div>
            <ArrowRight size={15} className="mx-auto rotate-90 text-gray-300 sm:rotate-0" />
            <div className="rounded-lg bg-[#fbeee1] px-3 py-2">
              <div className="text-[10px] font-bold uppercase tracking-wide text-[#b8611f]">Updated</div>
              <div className="mt-0.5 break-words text-sm font-semibold text-[var(--color-brand-navy)]">{formatAuditValue(change.new)}</div>
            </div>
          </div>
        </div>
      ))}
...
function MetadataRows({ values }: { values: Record<string, unknown> }) {
  ...
  if (!entries.length) return <div className="text-sm text-gray-500">No additional details recorded.</div>;
  return (
    <div className="divide-y divide-[var(--color-border)]">
      {entries.map(([key, value]) => (
        <div key={key} className="flex items-start justify-between gap-5 py-3 first:pt-0 last:pb-0">
          <span className="text-sm text-gray-500">{labelize(key)}</span>
          <span className="max-w-[65%] break-words text-right text-sm font-semibold text-[var(--color-brand-navy)]">{formatAuditValue(value)}</span>
        </div>
      ))}
    </div>
  );
...
function AuditTimelineDrawer({ row, onClose }: { row: AuditLogRow | null; onClose: () => void }) {
  if (!row) return null;
  return (
    <Drawer open={!!row} onClose={onClose} title={labelize(row.action)} subtitle={`${labelize(row.entity_type)} activity`} width="w-[680px] max-w-[calc(100vw-32px)]">
      <div className="space-y-4">
        ...
        <Card className="p-4">
          <div className="mb-2 text-sm font-bold text-[var(--color-brand-navy)]">Changed Fields</div>
          {Object.keys(row.changed_fields || {}).length ? (
            <ChangedFields fields={row.changed_fields || {}} />
          ) : (
            <div className="text-sm text-gray-500">No field-level diff recorded.</div>
          )}
        </Card>
        <Card className="p-4">
          <div className="mb-2 text-sm font-bold text-[var(--color-brand-navy)]">Metadata</div>
          <MetadataRows values={{ ...(row.metadata_json || {}), source: row.source }} />
        </Card>
      </div>
    </Drawer>
  );
  ...
  if (!rows.length) {
    return (
      <Card className="flex flex-col items-center justify-center p-10 text-center">
        ...
        <div className="font-bold text-[var(--color-brand-navy)]">No activity recorded yet</div>
          ...
          {rows.map((row, index) => (
            <button key={row.id} className="grid w-full grid-cols-[18px_1fr] gap-3 rounded-lg px-2 py-3 text-left transition hover:bg-accent-light/30" onClick={() => setSelected(row)}>
              <div className="relative flex justify-center">
                ...
                {index !== rows.length - 1 && <span className="absolute top-5 h-full w-px bg-[var(--color-border)]" />}
              </div>
              <div>
                <div className="text-sm font-bold text-[var(--color-brand-navy)]">{labelize(row.action)}</div>
